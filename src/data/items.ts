...
export const ITEMS: CustomizationItem[] = [
  // ─── Hats ────────────────────────────────────────────
...
export function itemsBySlot(slot: ItemSlot): CustomizationItem[] {
...
export const SLOT_BY_POSITION: ItemSlot[] = ['hat', 'outfit', 'badge', 'background'];

export function itemsForScript(slot: ItemSlot, script: ScriptId): CustomizationItem[] {
  return ITEMS.filter((i) => i.slot === slot && i.unlockedBy === script);
}

export function unlockedItemIds(scriptIds: ScriptId[]): Set<string> {
...
export function itemById(id?: string): CustomizationItem | undefined {
